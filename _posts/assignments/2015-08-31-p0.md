...
In order to get an idea of how Funcions and APIs are written in Javascript, perform a comparison of the [HTML5 Audio API](https://developer.mozilla.org/en-US/docs/Web/Guide/HTML/Using_HTML5_audio_and_video) and the [Android MediaPlayer API](http://developer.android.com/guide/topics/media/mediaplayer.html). Specifically we want you to compare the following tasks:

1. For each API, indicate how you would play the audio file hosted at `http://awesomemusic.com/sweet_track.mp3`
2. For each API, how would you pause, change the volume, and seek to a particular position in the audio file? 
3. For each API, how would you detect if there was an error loading the hosted file? 
4. For each API, how would you add an audio effect to the file being playeed (e.g Reverb or Compression)? You might have to take a closer look [here for HTML5](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API) and [here for Android](http://developer.android.com/reference/android/media/MediaPlayer.html).
5. Name one thing that would be harder to do in one of these APIs than the other and indicate why it would be harder. 
